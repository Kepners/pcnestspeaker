...
// DOM Elements
const tvDeviceSelect = document.getElementById('tv-device-select');
...
let selectedTvDevice = null;

/**
 * Populate TV device dropdown with discovered Cast devices
 * Filters for devices with cast_type === 'cast' (TVs, Shields, displays)
 */
function populateTvDeviceDropdown() {
  if (!tvDeviceSelect) return;

  // Clear existing options except the placeholder
  tvDeviceSelect.innerHTML = '<option value="">-- Select a TV --</option>';

  // Filter speakers for TV-compatible devices (cast_type === 'cast')
  const tvDevices = speakers.filter(speaker => {
    const castType = speaker.cast_type || 'audio';
    const model = (speaker.model || '').toLowerCase();
    const name = (speaker.name || '').toLowerCase();

    // Include: TVs, Shields, displays, Chromecasts with screens
    return castType === 'cast' ||
           model.includes('tv') ||
           model.includes('shield') ||
           model.includes('display') ||
           model.includes('chromecast') ||
           name.includes('tv');
  });

  // Add TV devices to dropdown
  tvDevices.forEach((device, index) => {
    const option = document.createElement('option');
    option.value = device.name;
    option.textContent = `${device.name} (${device.model || 'Chromecast'})`;
    option.dataset.ip = device.host || '';
    tvDeviceSelect.appendChild(option);
  });

  // Show message if no TV devices found
  if (tvDevices.length === 0) {
    const option = document.createElement('option');
    option.value = '';
    option.textContent = 'No TVs found on network';
    option.disabled = true;
    tvDeviceSelect.appendChild(option);
  }

  log(`TV dropdown: ${tvDevices.length} TV device(s) available`);
}

/**
 * Start TV streaming (HLS)
 */
async function startTvStreaming() {
  const deviceName = tvDeviceSelect ? tvDeviceSelect.value : null;

  if (!deviceName) {
    log('No TV selected', 'error');
    showError('Please select a TV first');
    return;
  }

  // Get device IP from data attribute
  const selectedOption = tvDeviceSelect.options[tvDeviceSelect.selectedIndex];
  const deviceIp = selectedOption ? selectedOption.dataset.ip : null;

  log(`Starting TV stream to ${deviceName}...`, 'info');
  showLoading('Starting TV stream...');

  try {
    const result = await window.api.startTvStreaming(deviceName, deviceIp);

    if (result.success) {
      tvStreamingActive = true;
      selectedTvDevice = deviceName;
      updateTvStreamingUI(true, deviceName);
      log(`TV streaming to ${deviceName}!`, 'success');
    } else {
      throw new Error(result.error || 'Failed to start TV streaming');
    }
  } catch (error) {
    log(`TV stream failed: ${error.message}`, 'error');
    showError(error.message);
  }
 ...
 * Stop TV streaming
 */
async function stopTvStreaming() {
  log('Stopping TV stream...', 'info');
  showLoading('Stopping TV stream...');

  try {
    const result = await window.api.stopTvStreaming();

    if (result.success) {
      tvStreamingActive = false;
      selectedTvDevice = null;
      updateTvStreamingUI(false);
      log('TV streaming stopped', 'success');
    } else {
      throw new Error(result.error || 'Failed to stop TV streaming');
    }
  } catch (error) {
    log(`TV stop failed: ${error.message}`, 'error');
    showError(error.message);
  }
...
/**
 * Update TV streaming UI
 */
function updateTvStreamingUI(streaming, deviceName = null) {
  // Update buttons
  if (tvStartBtn) {
    tvStartBtn.style.display = streaming ? 'none' : 'flex';
  }
  if (tvStopBtn) {
    tvStopBtn.style.display = streaming ? 'flex' : 'none';
  }

  // Update status indicator
  if (tvStatus) {
    tvStatus.style.display = streaming ? 'block' : 'none';
  }
  if (tvStatusText && deviceName) {
    tvStatusText.textContent = `Streaming to ${deviceName}...`;
  }

  // Disable device select while streaming
  if (tvDeviceSelect) {
    tvDeviceSelect.disabled = streaming;
  }
}

// TV Event Listeners
if (tvStartBtn) {
  tvStartBtn.addEventListener('click', startTvStreaming);
}

if (tvStopBtn) {
  tvStopBtn.addEventListener('click', stopTvStreaming);
}

// Populate TV dropdown when speakers are discovered
const originalRenderSpeakers = renderSpeakers;
renderSpeakers = function() {
  originalRenderSpeakers();
  populateTvDeviceDropdown();
};

// Handle TV visuals toggle
if (tvVisualsToggle) {
  tvVisualsToggle.addEventListener('change', (e) => {
    const enabled = e.target.checked;
    log(`TV ambient visuals: ${enabled ? 'ON' : 'OFF'}`);
    // This setting is read by the TV receiver when loading
    // The receiver checks URL parameter ?visuals=0 to disable
